...
To address these failure modes, we propose a two-part solution.
...
As introduced in Section~\ref{sec:challenges-solutions}, the control path---tool invocation, state management, and policy enforcement---requires server-side orchestration to reduce the trusted computing base (TCB).
This section argues why client-side orchestration is unsuitable and how the enforcement points map to the failure modes identified earlier.
...
Table~\ref{tab:enforcement} maps each failure mode from Section~\ref{sec:challenges-solutions} to the enforcement point that mitigates it.
...
Importantly, Llama Stack operates as a \emph{platform layer} rather than a client-side agent framework. Agent-building toolkits such as LangChain~\cite{langchain}, LangGraph~\cite{langgraph}, CrewAI~\cite{crewai}, and others provide developer-facing abstractions for authoring agents, but they orchestrate tool calls and inference from the client. Llama Stack provides the standardized API surface and server-side execution environment that these frameworks can target. Agents built with any OpenAI-compatible framework can interact with Llama Stack's endpoints unmodified, gaining server-side policy enforcement, multitenancy, and provider portability without changes to agent code.
...
The architecture supports several enterprise deployment patterns:

\begin{itemize}
  \item \textbf{Shared instance for regulated organizations.} Multiple business units share a single Llama Stack instance. ABAC policies ensure that each unit's documents, tool invocations, and conversation state remain isolated, avoiding the cost of per-unit infrastructure while satisfying compliance requirements.

  \item \textbf{Per-team distributions for platform teams.} A platform team offering AI-as-a-service internally uses the Kubernetes operator to provision per-team distributions in separate namespaces, combining namespace-level network isolation with application-level ABAC for defense in depth.

  \item \textbf{Progressive migration from development to production.} In both patterns, the provider abstraction allows organizations to start with lightweight backends (e.g., Ollama, sqlite-vec) during development and migrate to production-grade infrastructure (e.g., vLLM~\cite{kwon2023vllm}, pgvector) without modifying agent code or security policies---only the distribution configuration changes.
\end{itemize}
...
Several systems aim to standardize or simplify the development of LLM-based agentic applications. The closest to our work are API-layer platforms and enterprise agent runtimes that attempt to unify inference, tool use, and retrieval behind a common interface.

OpenAI's Responses API~\cite{openaiResponsesAPI} and function-calling conventions~\cite{openaiFunctionCalling} define a widely adopted interface for stateful, tool-using agents, and the Model Context Protocol (MCP)~\cite{mcp} standardizes how agents discover and invoke external tools. These specifications establish \emph{what} an agent can do but are silent on \emph{who} may do it---they assume a single-tenant caller and provide no authorization layer. Llama Stack adopts these interface conventions (OpenAI-compatible endpoints, MCP tool support) but adds server-side policy enforcement, tenant-scoped state, and provider-neutral execution beneath them.

The closest enterprise parallel is Databricks' Mosaic AI Agent Framework~\cite{databricksAgentFramework}, which wraps arbitrary agent code in an MLflow ResponsesAgent~\cite{mlflow,databricksResponsesAgent} for serving, tracing, evaluation, and monitoring, and connects agents to governed data through managed MCP servers backed by Unity Catalog. This provides production rails similar in spirit to our work, but couples the experience to the Databricks runtime and governance stack. Our goal in Llama Stack is to provide the same class of guarantees---multitenant isolation, auditable tool execution, retrieval gating---through an open, vendor-neutral API layer that operators can deploy on any infrastructure.

Other systems tackle specific layers of the problem. LLM serving engines such as vLLM~\cite{kwon2023vllm} and Orca~\cite{yu2022orca} optimize inference throughput but are agnostic to tenant isolation; Llama Stack treats them as pluggable providers beneath its authorization layer. Vector databases such as Milvus~\cite{milvus} and Weaviate~\cite{weaviate}, and end-to-end retrieval platforms such as Vectara~\cite{vectara}, provide dense, sparse, and hybrid search but rank by relevance without authorization enforcement; Llama Stack's retrieval-gating layer (Section~\ref{sec:challenges-solutions}) adds tenant-aware filtering on top of any such backend.

Agent orchestration frameworks---LangGraph~\cite{langgraph} for durable, stateful workflows; Microsoft's Agent Framework~\cite{microsoftAgentFramework} (successor to Semantic Kernel~\cite{semanticKernel} and AutoGen~\cite{autogen}) for typed, session-aware SDKs; Google's Agent Development Kit (ADK)~\cite{googleADK}; Haystack~\cite{haystack} for modular pipelines; LlamaIndex~\cite{llamaindex} for data-centric agent patterns; and others including Smolagents~\cite{smolagents} and Pydantic AI~\cite{pydanticai}---provide developer-facing abstractions for building agents. However, they orchestrate execution from the client or application process, placing security-critical logic outside a server-side trust boundary. Llama Stack is designed to be the server-side execution target that these frameworks call into, so that agents built with any OpenAI-compatible toolkit gain policy enforcement and tenant isolation without code changes.

Production ML systems research has documented the fragility that arises when critical invariants are distributed across components~\cite{sculley2015hiddentechnicaldebt,amershi2019se4ml}, and lifecycle platforms such as MLflow~\cite{mlflow} address reproducibility and deployment but do not target agentic multitenancy. To our knowledge, no prior work addresses the intersection of standardized agentic API design, server-side orchestration, and multitenant isolation on shared infrastructure---the gap this paper aims to fill.

\subsection{Future work}
Several directions remain open. First, formal verification of ABAC policy correctness---ensuring that policy compositions do not inadvertently permit cross-tenant access---would strengthen the guarantees beyond runtime enforcement. Second, quantitative benchmarking of isolation overhead (metadata filtering latency, authorization check cost per request, predicate pushdown efficiency across backends) is needed to characterize the performance trade-offs in production workloads. Third, multi-agent delegation patterns, where one agent invokes another on behalf of a different tenant, introduce authorization challenges that the current single-agent model does not address. Finally, extending the architecture to federated deployments---where Llama Stack instances in different administrative domains coordinate with consistent policy enforcement---would broaden applicability to cross-organizational use cases.
...
We formalized the relevance-authorization gap in agentic retrieval~\cite{karpukhin2020dpr,johnson2017faiss} and analyzed failure modes unique to autonomous agent architectures, including tool-mediated disclosure and client-side orchestration bypass.

Our layered isolation architecture demonstrates that server-side orchestration can serve as a unifying enforcement layer for secure agent behavior, centralizing retrieval, tool execution, and state management to maintain enterprise control over autonomous actions~\cite{sculley2015hiddentechnicaldebt,amershi2019se4ml}. The approach involves trade-offs---notably ABAC policy complexity and variable filtering performance across backends---that Section~\ref{sec:implementation} discusses.
