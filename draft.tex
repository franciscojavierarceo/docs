...
\begin{enumerate}
  ...
  \item We argue that server-side orchestration provides a natural enforcement layer for security-critical operations---centralizing authorization, state isolation, and audit logging---while client-side frameworks retain control over agent composition, enabling hybrid architectures that reduce the trusted computing base without sacrificing flexibility~\cite{sculley2015hiddentechnicaldebt,amershi2019se4ml}.

  \item We validate the proposed architecture through an implementation in Llama Stack~\cite{llamastack}, an open-source, vendor-neutral framework with Kubernetes deployment via an operator~\cite{llamastackk8soperator}, demonstrating the feasibility of secure multitenant agentic RAG on shared infrastructure with pluggable models, vector stores, and tools.
\end{enumerate}
...
\textbf{Layer 3: Shared inference.} LLM layer shared across tenants with isolation enforced at input construction, reducing cost from $O$($N \cdot M$) to $O$($M$), where $N$ is the number of tenants and $M$ is the number of model endpoints.
...
When the client controls the inference $\rightarrow$ tool $\rightarrow$ inference loop, a compromised or buggy client can skip retrieval filters, invoke unauthorized tools, or accumulate cross-tenant context.
...
Figure~\ref{fig:orchestration-flow} illustrates this server-side execution model where every step—from request processing through tool execution to response storage—occurs within the server trust boundary. Table~\ref{tab:enforcement} maps each shortcoming from Section~\ref{sec:challenges-solutions} to the enforcement point that mitigates it.
...
Llama Stack implements the Responses API not only as an agentic execution endpoint but as a resource model encompassing vector stores, files, and conversations---each a first-class, tenant-scoped API object subject to the same ABAC policies as responses themselves (Figure~\ref{fig:openai-responses-api}).
...
The implementation provides vendor-neutral abstractions where third-party providers can be supported through Llama Stack's pluggable architecture. This enables organizations to use different providers for each capability based on their requirements, avoiding vendor lock-in while maintaining consistent interfaces. Figure~\ref{fig:openai-responses-api} illustrates the complete surface area and interconnections between these APIs. The key APIs and their relationships include:

\begin{itemize}
  \item \textbf{Responses API:} The central orchestration endpoint that unifies multi-turn conversations, tool calling, and agentic execution. Implements OpenAI's Responses API paradigm~\cite{openaiResponsesAPI} with server-side state management and policy enforcement.

  \item \textbf{Conversations API:} Manages multi-turn conversation state and history with tenant isolation. Handles conversation persistence, retrieval, and access control, ensuring that conversation context remains within appropriate authorization boundaries across turns.

  \item \textbf{Search API:} Provides unified search capabilities across multiple retrieval modalities including vector search, keyword matching, and hybrid approaches. Integrates with Vector Stores API and supports structured filtering for tenant isolation and policy enforcement at retrieval time.

  \item \textbf{Vector Stores API:} Abstracts vector database operations across multiple providers including Chroma, pgvector, Elasticsearch, Qdrant, Weaviate, Milvus, Oracle Cloud Infrastructure, and sqlite-vec. Supports both dense vector search and hybrid retrieval with metadata-based filtering for multitenant isolation.

  \item \textbf{Files API:} Manages file storage and retrieval with support for multiple backend providers including cloud storage (Google Cloud Storage, AWS S3), distributed storage (Persistent Volume Claims), and local storage. Provides tenant-scoped file access with ABAC policy enforcement.

  \item \textbf{File Search Tool:} Implements semantic search over document collections by combining the Files API with vector-based retrieval through the Search API. Enables agents to query enterprise document repositories with tenant isolation and access control.

  \item \textbf{Tools:} Extensible tool execution framework supporting both built-in tools (image generation, function calling, web search, file search, code interpreter, computer use) and external tools via Model Context Protocol (MCP). All tool execution occurs server-side with authorization validation and audit logging.

  \item \textbf{Prompts API:} Manages prompt templates and configurations with full programmatic access via API.
\end{itemize}
...
\begin{scope}[on background layer]
  % Client to Files API (drawn on deepest layer, underneath everything)
...
% Inference Providers label (above the provider group)
\node[lab, anchor=south] at ([yshift=11mm]responsesPos) {Inference Providers};

% Draw API boxes on top (main layer)
...
% Legend aligned with Tools box horizontally, trimmed margins
\begin{scope}[on background layer]
...
% Tools area (moved up further to reduce whitespace)
\begin{scope}[on background layer]
...
% Top row - more evenly spaced with extra room
...
% Bottom row - evenly spaced within the Tools box
...
% Arrows
% Client to Responses API (main red arrow)
\draw[->, red, line width=1.5pt] (client.south) -- (responses.north);

% Client to Conversations API (direct connection with 90-degree angle, parallel to Responses->Convos)
\draw[->, thick] ([yshift=-4mm]client.west) -- (4.5,10.8) -- (4.5,5.5);

% Client to Search API (direct connection with 90-degree angle, parallel to Responses->Search)
\draw[->, thick] ([yshift=-4mm]client.east) -- (17.5,10.8) -- (17.5,5.51);

% Client to Vector Stores API (direct connection with 90-degree angle, parallel to Responses->Vector Stores)
\draw[->, thick] ([yshift=-4mm]client.east) -- (13.,10.8) -- (13.,5.45);


% Responses API to Prompts API (orthogonal: left then down)
\draw[->, thick] (responses.west) -- (0,8.5) -- (prompts.north);
\node[lab] at (0.6,8.1) {Read};

% Responses API to Conversations API (orthogonal: left then down)
\draw[->, thick] (responses.west) -- (4.2,8.5) -- (convos.north);
\node[lab] at (4.2,8.1) {Read/Write};



% Responses API to Tools (Tool Call - green arrow: right, around Search API, down to Tools)
...
% Files API to Vector Stores API (horizontal)
\draw[->, thick] (files.east) -- node[lab, above] {Read} (vectors.west);

% File Processor to Files API (from left side, orthogonal: left then up to Files)
\draw[->, dashed, thick] (fp.west) -- (8,2.5) -- (files.south);
\node[lab] at (7.3,2.8) {Read};

% File Processor to Vector Stores API (from right side, orthogonal: right then up to Vectors)
\draw[->, dashed, thick] (fp.east) -- (13,2.5) -- (vectors.south);
\node[lab] at (13.7,2.8) {Write};

% Vector Stores to Search API (horizontal)
\draw[->, thick] (vectors.east) -- node[lab, above] {Read} (search.west);

% Search API to File Search (red dashed line with 90-degree angles)
\draw[->, red, dashed, thick] (search.south) -- (17.5,3) -- (15.9,3) -- (tFileS.north);

\end{tikzpicture}%
}
\caption{Llama Stack's Responses surface area and its relation to other APIs, providers, and tools.}
\Description{Diagram showing the OpenAI Responses API architecture with client, inference providers, file and vector store providers, file processor, and tools.}
