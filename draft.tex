...
A fundamental security gap underlies existing agentic architectures: dense retrieval ranks documents by semantic similarity, not authorization---enabling cross-tenant data leakage when agents retrieve similar but unauthorized content. We formalize this similarity-authorization gap and analyze additional failure modes unique to agentic systems: tool-mediated disclosure where agents access unauthorized data through tool invocation, context accumulation across conversation turns, and client-side orchestration bypass. We introduce a layered isolation architecture combining policy-aware ingestion, retrieval-time gating, and shared inference, enforced through server-side agentic orchestration that centralizes tool execution, state management, and policy enforcement, creating natural enforcement points for secure agent behavior.
...
Llama Stack deployments are managed via a Kubernetes operator that automates lifecycle management on shared infrastructure. The operator supports multiple deployment topologies from shared instances with logical isolation to per-tenant deployments with namespace isolation, achieving cost efficiency through shared backends while maintaining security through application-level policy enforcement.
