...
We validate the proposed architecture through an open-source implementation in Llama Stack\footnote{\url{https://github.com/llamastack/llama-stack}}, a vendor-neutral framework that implements an OpenAI-compatible, open-source Responses API with server-side multi-turn orchestration. We evaluate it empirically and show that ABAC gating eliminates cross-tenant leakage while introducing negligible overhead.
\end{abstract}
...
\begin{enumerate}
  ...
  \item We implement the architecture in Llama Stack~\cite{llamastack}, an open-source, vendor-neutral framework providing OpenAI-compatible APIs with pluggable providers for inference, vector stores, and tools, deployable on Kubernetes via a dedicated operator~\cite{llamastackk8soperator}.
\end{enumerate}
...
We evaluate the architecture through six experiments measuring security, systems performance, and retrieval quality. The evaluation uses a 2$\times$2 configuration matrix crossing orchestration mode (client-side vs.\ server-side) against retrieval gating (ungated vs.\ ABAC-gated):
...
\begin{table}[!htbp]
  ...
  \bottomrule
  \end{tabular}
  \caption{Security metrics across configurations. Gating eliminates cross-tenant leakage entirely (Configs B and D), validating Equation~(2). Server-side orchestration alone (Config C) does not prevent leakage.}
  \label{tab:security}
\end{table}
...
\textbf{Latency overhead.} Isolating the search component from inference, the gated search path adds ${\sim}$19ms: auth server round-trip (${\sim}$14ms), ABAC policy evaluation ($<$1ms), and per-tenant store lookup (${\sim}$5ms). This is negligible relative to inference time (${\sim}$3--7s for API-based, ${\sim}$450ms for self-hosted GPU). On self-hosted GPU infrastructure (vLLM on T4), the Llama Stack routing and dispatch layer adds 4.7ms (1.0\% of baseline inference), and tenant metadata filtering adds 5.5ms (1.9\% of search time). Full latency tables are in Appendix~\ref{sec:latency-tables}.

Server-side orchestration adds ${\sim}$3s total latency vs.\ client-side due to the Responses API tool execution round-trip. All experiments used non-streaming responses (full completion before returning). With streaming---supported by Llama Stack's Responses API---time-to-first-token would be substantially lower.
...
\begin{itemize}
  ...
  \item \textbf{Predicate pushdown.} As shown in Section~\ref{sec:evaluation}, backends without native predicate pushdown experience recall degradation at scale. Llama Stack enforces post-retrieval filtering across all backends regardless of native pushdown support, so the security guarantee holds; the trade-off is recall, not security.
...
Our implementation through Llama Stack demonstrates that secure multitenancy, vendor-neutral OpenAI-compatible APIs, and autonomous agent capabilities are simultaneously achievable on shared infrastructure without per-tenant duplication.
...
% Title
\node[font=\LARGE\bfseries] (title) at (8,12.5) {\underline{Responses API Surface Area}};
...
\draw[->, thick] (responses.west) -- (2,8.5) -- (2,3.5) -- (compact.north);
...
\caption{Responses API surface area and its relation to other APIs, providers, and tools. The Responses API serves as the central orchestration endpoint, coordinating inference, tool execution, state management, and context compaction.}
...
\twocolumn
Llama Stack implements the Responses API not only as an agentic execution endpoint but as a resource model encompassing vector stores, files, and conversations---each a first-class, tenant-scoped API object subject to the same ABAC policies. The key APIs include:

\begin{itemize}
  \item \textbf{Responses API:} Central orchestration endpoint unifying multi-turn conversations, tool calling, and agentic execution via the OpenAI Responses API paradigm~\cite{openaiResponsesAPI}.

  \item \textbf{Inference API:} Chat completions and embeddings with OpenAI-compatible endpoints, supporting streaming, structured output, and tool use.

  \item \textbf{Vector Stores and Search APIs:} Vector store management and unified search across dense, sparse, and hybrid retrieval modalities with structured metadata filtering for tenant isolation.

  \item \textbf{Conversations API:} Multi-turn conversation state with tenant isolation, ensuring context remains within authorization boundaries across turns.

  \item \textbf{Compaction API:} Context management for long conversations. When token count exceeds a configurable threshold, compaction summarizes conversation history via inference while preserving user messages verbatim, then feeds the compacted context back as input. Supports both explicit compaction (\texttt{POST /v1/responses/compact}) and automatic compaction via the \texttt{context\_management} parameter on response creation.

  \item \textbf{Prompts API:} Versioned prompt template management with tenant-scoped access control.

  \item \textbf{Files and File Processors APIs:} File storage with backend support for cloud storage (GCS, S3), distributed storage (PVCs), and local storage, with tenant-scoped ABAC enforcement. File processors handle parsing and chunking for ingestion into vector stores.

  \item \textbf{Safety API:} Input and output content moderation via configurable shield providers.

  \item \textbf{Tools:} Extensible tool execution supporting built-in tools (file search, web search, code interpreter, image generation, computer use) and external tools via MCP~\cite{mcp}. All server-side tool execution occurs with authorization and audit logging.

  \item \textbf{Telemetry API:} Structured logging, tracing, and metrics for observability and compliance auditing.

  \item \textbf{Batches API:} Asynchronous batch processing for high-throughput workloads.

  \item \textbf{Compatibility layers:} Anthropic Messages API and Google Interactions API compatibility for multi-provider portability.
\end{itemize}

\noindent Supported inference providers include vLLM~\cite{kwon2023vllm}, Ollama, and OpenAI (remote). Vector store providers include Chroma, pgvector, Elasticsearch, Qdrant, Weaviate, Milvus, Oracle Cloud Infrastructure, FAISS~\cite{johnson2017faiss}, and sqlite-vec. The Kubernetes Operator~\cite{llamastackk8soperator} enables deployment of heterogeneous backends as shared services, with multiple Llama Stack instances referencing the same providers while maintaining logical isolation through ABAC.
