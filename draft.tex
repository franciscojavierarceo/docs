...
% Legend
\node[boxSpec, minimum width=20mm, minimum height=7mm, anchor=west, fill=white] (legA) at (-1.5,-1.3) {};
\node[lab, anchor=west] at ([xshift=2mm]legA.east) {OpenAI Spec Available};
\node[boxKnown, minimum width=20mm, minimum height=7mm, anchor=west] (legB) at (-1.5,-2.2) {};
\node[lab, anchor=west] at ([xshift=2mm]legB.east) {Closed Source (Known)};
\node[boxRed, minimum width=20mm, minimum height=7mm, anchor=west] (legC) at (-1.5,-3.1) {};
\node[lab, anchor=west] at ([xshift=2mm]legC.east) {Closed Source (Unknown)};

% Tools area (made wider for better spacing)
\begin{scope}[on background layer]
\fill[ToolsPurple!45, rounded corners=10pt] (4.2,-4.2) rectangle (17.3,-0.6);
\draw[SpecBlue!40!black, rounded corners=10pt, line width=0.8pt] (4.2,-4.2) rectangle (17.3,-0.6);
\end{scope}
\node[font=\large\bfseries] at (10.75,-0.95) {Tools};
% Top row - more evenly spaced with extra room
\node[tool] (tImg)   at (5.6,-1.9) {Image Generation};
\node[tool] (tFunc)  at (8.2,-1.9) {Function Calling};
\node[tool] (tMcp)   at (10.75,-1.9) {Remote MCP};
\node[tool] (tWeb)   at (13.3,-1.9) {Web Search};
\node[tool] (tFileS) at (15.9,-1.9) {File Search};
% Bottom row - centered within the Tools box
\node[tool, minimum width=24mm] (tCI)    at (7.5,-3.1) {Code Interpreter};
\node[tool, minimum width=22mm] (tCU)    at (9.8,-3.1) {Computer use};
\node[tool, minimum width=20mm] (tPatch) at (12.1,-3.1) {Apply patch};
\node[tool, minimum width=18mm] (tShell) at (14.2,-3.1) {Shell};
...
\draw[->, ToolGreen, line width=2pt, rounded corners=8pt] ([yshift=2mm]responses.east) -- (19,8.7) -- (19,-2.5) -- (17.3,-2.5);
...
\draw[->, dashed, thick] (fp.west) -- (8,1.2) -- (files.south);
\node[lab] at (7.3,1.5) {Read};

% File Processor to Vector Stores API (from right side, orthogonal: right then up to Vectors)
\draw[->, dashed, thick] (fp.east) -- (13,1.2) -- (vectors.south);
\node[lab] at (13.7,1.5) {Write};
