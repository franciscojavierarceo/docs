...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[
    layer/.style={draw, rounded corners=4pt, minimum width=72mm, minimum height=11mm,
    ...
    \node[layer, fill=KnownBlue!40] (L1) {Layer 1: Policy-Aware Ingestion};
    \node[layer, fill=ToolsPurple!40, above=5mm of L1] (L2) {Layer 2: Retrieval Gating};
    \node[layer, fill=ToolYellow!60, above=5mm of L2] (L3) {Layer 3: Shared Inference};

    % Sub-labels
    \node[sublabel, below=0.5mm of L1.south] (L1s) {$\mathcal{I}(d,t) \rightarrow D_t$ \quad tenant metadata at ingest};
    \node[sublabel, below=0.5mm of L2.south] (L2s) {ABAC + metadata filters \quad predicate pushdown};
    \node[sublabel, below=0.5mm of L3.south] (L3s) {shared model, tenant-scoped context};

    % Arrows between layers
    \draw[arrow] (L1) -- node[right, font=\scriptsize] {tagged chunks} (L2);
    \draw[arrow] (L2) -- node[right, font=\scriptsize] {authorized context} (L3);

    % Server-side orchestration bracket
    \node[draw=ToolGreen, very thick, rounded corners=6pt, dashed,
          fit=(L1)(L1s)(L2)(L2s)(L3)(L3s),
...
\subsection{Prompt injection probes}
...
\subsection{Predicate pushdown scaling}
...
\subsection{Retrieval quality (synthetic embeddings)}
...
\noindent Figure~\ref{fig:openai-responses-api} illustrates the complete Responses API surface area and the interconnections between APIs, providers, and tools.
...
\caption{Responses API surface area and its relation to other APIs, providers, and tools. The Responses API serves as the central orchestration endpoint, coordinating inference, tool execution, and state management.}
\Description{Diagram showing the Responses API architecture with client, inference providers, file and vector store providers, file processor, and tools.}
