...
A fundamental security gap underlies existing agentic architectures: dense retrieval ranks documents by semantic similarity, not authorization---enabling cross-tenant data leakage when agents retrieve similar but unauthorized content.
We formalize this similarity-authorization gap and analyze additional failure modes unique to agentic systems: tool-mediated disclosure where agents access unauthorized data through tool invocation, context accumulation across conversation turns, and client-side orchestration bypass.
We introduce a layered isolation architecture combining policy-aware ingestion, retrieval-time gating, and shared inference, enforced through server-side agentic orchestration that centralizes tool execution, state management, and policy enforcement, creating natural enforcement points for secure agent behavior.
...
Enterprise AI deployment has evolved toward agentic systems---autonomous AI applications that reason, use tools, retrieve information, and execute multi-step workflows to accomplish complex tasks~\cite{yao2023react,schick2023toolformer,karpas2022mrkl}. This represents a fundamental shift from language models as text generators to reasoning engines capable of autonomous action in enterprise environments.

The Responses API paradigm~\cite{openaiResponsesAPI,openresponses} unifies multi-turn inference, tool use, and retrieval behind a single endpoint, enabling stateful agentic workflows. Open implementations increasingly expose compatible endpoints to decouple applications from proprietary providers, enabling enterprise control over autonomous agent behavior.
...
Serving efficiency and scheduling are handled by the inference backend; modern LLM serving systems show that batching and memory management dominate throughput~\cite{yu2022orca,kwon2023vllm}. Recent work demonstrates that smaller, specialized models can achieve competitive agentic performance while offering significant deployment advantages in enterprise contexts~\cite{chen2025small}.
...
The patterns presented---server-side orchestration, policy-aware retrieval, and operator-driven infrastructure---establish a foundation for secure, scalable autonomous agent deployments with systematic behavioral control.
