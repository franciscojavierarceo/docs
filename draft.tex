...
\textbf{Retrieval infrastructure} evolved to support multiple search modalities: dense vector search across diverse vector databases (FAISS~\cite{johnson2017faiss}, Elasticsearch, Qdrant, Weaviate, ChromaDB, PGVector, Milvus), sparse keyword matching (BM25), and hybrid approaches combining both with neural rerankers~\cite{blendedrag2024}. Advanced chunking strategies including contextual enrichment~\cite{contextualchunking2024} enhanced document segmentation for improved retrieval coherence.
...
We propose a three-layer architecture that directly addresses the failure modes identified in Section~\ref{sec:challenges-solutions}: policy-aware ingestion, retrieval-time gating, and shared inference.
...
\textbf{(1) Resource-level ABAC:} before any search, the system checks that the user is authorized to read the search store; if not, no search is performed.
...
LlamaStack supports vector search, keyword search (BM25), and hybrid approaches that combine both with optional reranking. Regardless of search methodology, authorization is applied consistently: each retrieval path first filters the document corpus by tenant and resource-level policies before applying relevance ranking, ensuring that all search modalities respect the same isolation boundaries. This implements the secure retrieval formulation from Equation~(1).
Backends such as Elasticsearch, PGVector, and other SQL-backed vector databases can support predicate pushdown for efficiency on large corpora; otherwise, post-retrieval gating still enforces the relevance/authorization separation across all provider types.
...
Server-side orchestration is essential for multitenant security: it centralizes retrieval, tool execution, and state management, reducing the trusted computing base and addressing failure modes in Section~\ref{sec:challenges-solutions}.
...
Retrieval uses query-time predicates (tenant predicates and classification filters) to prevent cross-tenant candidate leakage, separating relevance ranking from authorization checks across all supported vector database providers~\cite{karpukhin2020dpr,johnson2017faiss}.

\subsection{Two-phase retrieval: search and admission}
Phase~1 performs retrieval with authorization gating across multiple search modalities. LlamaStack supports vector search, keyword search (BM25), and hybrid approaches that combine both with optional reranking~\cite{karpukhin2020dpr,johnson2017faiss}.
