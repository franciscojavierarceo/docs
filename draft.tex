...
\begin{tikzpicture}[
  ...
  boxSpec/.style={box, draw=SpecBlue, text=SpecBlue, fill=white, line width=1.2pt},
...
% Title
\node[font=\LARGE\bfseries] (title) at (8,12.5) {\underline{OpenAI Responses API}};
...
% Bottom row - centered within the Tools box
\node[tool, minimum width=24mm] (tCI)    at (7.5,-1.4) {Code Interpreter};
\node[tool, minimum width=22mm] (tCU)    at (9.8,-1.4) {Computer use};
\node[tool, minimum width=20mm] (tPatch) at (12.1,-1.4) {Apply patch};
\node[tool, minimum width=18mm] (tShell) at (14.2,-1.4) {Shell};
...
\draw[->, thick] ([yshift=-4mm]client.east) -- (17.8,10.8) -- (17.8,5.51);

% Client to Vector Stores API (direct connection with 90-degree angle, parallel to Responses->Vector Stores)
\draw[->, thick] ([yshift=-4mm]client.east) -- (13.3,10.8) -- (13.3,5.45);
...
\draw[->, ToolGreen, line width=2pt, rounded corners=8pt] ([yshift=2mm]responses.east) -- (19,8.7) -- (19,-0.8) -- (17.3,-0.8);
\node[font=\bfseries, text=ToolGreen, anchor=south] at (14,8.7) {Tool Call};
...
\caption{OpenAI Responses API relationships to related APIs, providers, and tools.}
