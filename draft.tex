...
We validate the proposed architecture through an open sourced implementation in Llama Stack\footnote{\url{https://github.com/meta-llama/llama-stack}}---a vendor-neutral framework realizing the Responses API paradigm with server-side multi-turn orchestration---and evaluate it empirically, demonstrating that ABAC gating eliminates cross-tenant leakage entirely while adding negligible overhead (${\sim}$19ms on the search path).
\end{abstract}
