...
% Draw provider groups on background layer FIRST
...
% Responses API to Files API (straight down)
\draw[->, thick] (responses.south) -- (files.north);

% Responses API to Vector Stores API (orthogonal: right then down)
\draw[->, thick] (responses.east) -- (13,8.5) -- (vectors.north);
\node[lab] at (12.2,8.1) {Read};

% Responses API to Search API (orthogonal: right then down)
\draw[->, thick] (responses.east) -- (17.5,8.5) -- (search.north);
\node[lab] at (16.8,8.1) {Read};
...
% Search API to File Search (red dashed line straight down)
\draw[->, red, dashed, thick] (search.south) -- (tFileS.north);
